...
const AdminCourses = () => {
  const [courses, setCourses] = useState([]);
  const [editCourse, setEditCourse] = useState(null);
  const [openAdd, setOpenAdd] = useState(false); // ✅ Control add course dialog
  const [newCourse, setNewCourse] = useState({
    name: "",
    price: "",
    instructor: "",
  ...
  const onChangeNew = (k) => (e) => {
    if (k === "image") setNewCourse({ ...newCourse, image: e.target.files[0] });
    else setNewCourse({ ...newCourse, [k]: e.target.value });
  };
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`courses/${id}/delete/`); fetchCourses(); }
    catch (err) { console.error(err); }
  };
  ...
  return (
    <div>
          ...
          <TableBody>
            {courses.map((course) => (
              <TableRow key={course.id}>
                ...
                <TableCell>{course.duration}</TableCell>
                <TableCell>
                  <Button color="primary" size="small" onClick={() => setEditCourse(course)} sx={{ mr:1 }}>Edit</Button>
      ...
      {editCourse && (
        <Dialog open={true} onClose={() => setEditCourse(null)}>
          {/* ...same as your current edit modal... */}
        </Dialog>
      )}
    </div>
  );
